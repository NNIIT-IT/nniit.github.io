(function (window)
{
 ...
 $(document).ready(
		function(){ 
			$('#slider_main').slick({ dots: false, autoplay: true, slidesPerRow:1, autoplaySpeed: 15000, adaptiveHeight: false});

			/*$('.1main_slider').slick({
				slidesToShow:<?=$cntslide?>,
				slidesToScroll: <?=$cntslide?>,
				responsive: [
				<?foreach($arResult["ITEMS"] as $arItem):?>
					...
					<?$posicon=940-$cnt*120;?>
						...
						breakpoint: <?=$posicon?>,
						settings: {
							slidesToShow: <?=$cntslide-$cnt?>,
							slidesToScroll: <?=$cntslide-$cnt?>,
							}
						},
					<?endif;$cnt++;?>
				<?endforeach;?>
