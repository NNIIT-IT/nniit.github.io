...
while ($row = $result->fetch())
{
	foreach ($row as $k => $v)
	{
		...
		$html = call_user_func_array(
			array($arUserField['USER_TYPE']['CLASS_NAME'], 'GetAdminListViewHTML'),//GetAdminListViewHTML
			array(
				$arUserField,
				array(
...
*/
		...
		if($arUserField['USER_TYPE']['BASE_TYPE']=="file") {
				$arfile=CFile::GetFileArray(intval($v));
		}
		//$value= htmlspecialcharsbx(is_array($v) ? implode(', ', $v) : $v);
		$row[$k]=array(
					...
					'BASE_TYPE'=>$arUserField['USER_TYPE']['BASE_TYPE'],
					'VALUE' =>($arUserField['USER_TYPE']['BASE_TYPE']=="file")?$arfile:htmlspecialcharsbx(is_array($v) ? implode(', ', $v) : $v),
				);
		//	$row[$k] = $html;
