...
class purposeCab extends  iasmuinfo{
	...
	const  arTables=array(
		"purposeCab"=>array(
			...
			"th"=>array("Адрес места нахождения",
...
$listitemsTmp=array();
	...
	//object data
	$arFilter=array("IBLOCK_ID"=>$IBLOCK_SOBSTV,"ACTIVE"=>"Y","ID"=>array_keys($objListId));
	...
	//type ovz 
	$filter=array();
	if(count($objOvzId)>1) $filter=array("PROPERTY_ID"=>117,"IBLOCK_ID"=>$IBLOCK_CAB,"ID"=>array_keys($objOvzId)); 
	if(count($objOvzId)==1) $filter=array("PROPERTY_ID"=>117,"IBLOCK_ID"=>$IBLOCK_CAB,"ID"=>array_keys($objOvzId)[0]);
	...
	//type auditor	
	if(is_array($objAudId) && count($objAudId)==1) $filter=array("IBLOCK_ID"=>$IBLOCK_CAB,"ID"=>array_keys($objAudId)[0]); 
	if(is_array($objAudId) &&count($objAudId)>1) $filter=array("IBLOCK_ID"=>$IBLOCK_CAB,"ID"=>array_keys($objAudId));
	...
	foreach($listitemsTmp as $k=>$Citem){
		...
		//данные только для инвалидов && $this->ovz
		if(($ovz=="Inv2")) {
		...
		//данные только для не инвалидов && !$this->ovz
		if(($ovz=="Inv0") ){
			$mainKey="purpose".$addkey;
			//$xitem["ovz".$addkey]=$ovzName;
			$thislistitems[$mainKey][$k]=$xitem;
...
public function setparams($params){
	...
	$this->ovz=false;
	if (isset($params["ovz"]) && ($params["ovz"]==1)){
		$this->ovz=true;
	}
	if(count($this->activeTable)==0) $this->activeTable=$arPropIDs;
	
	$this->cashGUID=md5("purpose".implode(",",$this->activeTable).($this->ovz?'1':'0'));
...
public function showHtml($buffer=true){
	...
	if ($html=="" || $html==false || $this->isEditmode()){
		...
		foreach($this->activeTable as $tableID){
			...
			//echo "<div style=\"display:block\"><pre>";print_r($tableRows);echo"</pre></div>";
			if(!is_array($tableRows) || count($tableRows)==0){
				//заголовок таблицы
					$html.="<span class=\"texticon hidedivlink link\" >".$tableData["name"]."</span>";
					$html.="<div style=\"display:block;\"><table class=\"simpletable\"><thead><tr>";
				...
				//заголовок таблицы
					...
					foreach($tableRows as $tableRow){
						$tableRowID=(($this->ovz)?"ovz":"").$tableID.$tableRow['id'];
						$html.="<!--".$tableID."--><tr itemprop=\"{$tableData['itemprop']}\" id=\"{$tableRowID}\" ";
						$html.=" data-id=\"{$tableRow['id']}\" data-iblock=\"{$IBLOCKID}\" class=\"maindocselement\" title=\"{$tableRow['nameCab']}\">";
						foreach($tableData["td"] as $td){
							$elHtm=$tableRow[$td];
							if(($td=="osnPrac" || $td=="osnCab" || $td=="ovzPrac" || $td=="ovzCab")&& (strlen($elHtm)>20)){
								//$html.="<td ><span class=\"texticon hidedivlink link\">Показать</span><div style=\"display:none;\" itemprop=\"{$td}\">{$elHtm}</div></td>";
								$html.="<td ><div style=\"font-size:0.9em;\" itemprop=\"{$td}\">{$elHtm}</div></td>";
							}else{
								if($elHtm=="") $elHtm="Отсутствует";
