...
use asmuinfoclasses;
class resultVI1 extends  iasmuinfo{
	const cashTime=600;
	...
	function setparams($params){
		...
		$this->cashGUID="resultVI1_".$this->eduLevelId;
	...
	function generateItems(){
		...
		$this->examsList=array();
		$sql="";
		$asEduLevelStr=array(876=>"Р‘Р°РєР°Р»Р°РІСЂРёР°С‚",877=>"РЎРїРµС†РёР°Р»РёС‚РµС‚",879=>"РћСЂРґРёРЅР°С‚СѓСЂР°",879=>"РђСЃРїРёСЂР°РЅС‚СѓСЂР°");
		$god=$this->god;
		...
		$arSqlWhere=array();
		$sql="SELECT distinct UF_USER_FIO,UF_EXAMS, UF_NUMBER_ZAYAV,UF_SNILS FROM `abit_list_by_fio_test` ";
		...
		if($rez=$this->BD->query($sql)){
			while ($rec=$rez->fetch()){
				...
				$dataRow=array("UF_USER_FIO"=>$rec["UF_USER_FIO"],"UF_EXAMS"=>$examsValues, "UF_NUMBER_ZAYAV"=>$rec["UF_NUMBER_ZAYAV"],"UF_SNILS"=>$rec["UF_SNILS"]);
	...
	public function printTables(){
		...
		if(count($this->listitems)>0){
			$html="<div class=\"resultVITable\" itemprop=\"resultVI\">";
				...
				$html.="<div class=\"resultVICell\">РќРѕРјРµСЂ Р·Р°СЏРІР»РµРЅРёСЏ / РЎРќР