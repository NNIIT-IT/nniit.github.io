...
use asmuinfoclasses;
class maindocsA extends  iasmuinfo{
	private $cashGUID;
	const cashTime=6000;
	const emptyLink="Отсутствует";
	const emptyFile="/sveden/emptyFile.pdf";
	const arBlocks=array(67);
	...
	foreach($this->propList as $itemprop){
		...
		if(!$isPrezent && $itemprop!="localAct" && $itemprop!="priemLocalAct"){
			$tmpItems[$itemprop]=array(
				...
				"groupNameL1"=>$arPropListData[$itemprop]["UF_ORG_UNIT"],
				...
				"name"=>$arPropListData[$itemprop]["UF_NAME"],
...
//	$item=array("name"=>$rec["itemName"],"fname"=>"","id"=>$rec["id"],"itemType"=>$rec["itemType"],"itemText"=>$rec["itemText"],"itemHide"=>$rec["itemHide"]);
			...
			case 4://файл
				...
				if($fileSRC!=""){
					$rez.="<a {$itemprop} title=\"{$elementname}\" target=\"blank\" download class=\"linkicon {$adClass}\" id=\"{$idlink}\"  data-section=\"{$defaultSection}\" href=\"{$fileSRC}\" data-fileid=\"{$element["fileId"]}\" data-id=\"{$element["id"]}\" data-iblock=\"67\" data-xmlId=\"{$element["xmlId"]}\" {$sprops}>{$elementname}</a>{$sigreg}{$sigfile}<br>\r\n";	
					}	
				else {
					$fileSRC=$this::emptyFile;
					$rez.="<a {$itemprop}  data-section=\"{$defaultSection}\" title=\"{$elementname}\" class=\"iconEmpty {$adClass}  \" id=\"{$idlink}\" data-id=\"{$element["id"]}\" href=\"{$fileSRC}\" data-iblock=\"67\" data-xmlId=\"{$element["xmlId"]}\" {$sprops} >{$elementname} - ".$this::emptyLink."</a><br>\r\n";	
				}
				$rez.="\r\n";
				break;
...
break;*/
			case 17://php object
				...
				if($clName!=""){
					if(file_exists($_SERVER["DOCUMENT_ROOT"]."/sveden/class/cl-".$clName."/cl-".$clName.".php")){
						...
						$rez="<span title=\"{$elementname}\" class=\"texticon hidedivlink linkicon {$adClass}\" id=\"{$idlink}\" data-id=\"{$element["id"]}\" data-iblock=\"67\" data-xmlId=\"{$element["xmlId"]}\">{$elementname}</span>\r\n";
				...
				if($rez==""){
					if($element["itemText"]!=""){
						$rez.="<span title=\"{$elementname}\" class=\"texticon hidedivlink linkicon {$adClass}\" id=\"{$idlink}\" data-id=\"{$element["id"]}\" data-iblock=\"67\" data-xmlId=\"{$element["xmlId"]}\" {$sprops}>{$elementname}</span>\r\n";
						$rez.="<div style=\"{$adStyle} padding:1em;\" class=\"\" {$itemprop}>{$element["itemText"]}</div><br>\r\n";
					}else{
						$rez.="<span {$itemprop} title=\"{$elementname}\" class=\"linkicontxt {$adClass} \" id=\"{$idlink}\" data-id=\"{$element["id"]}\" data-iblock=\"67\" data-xmlId=\"{$element["xmlId"]}\" {$sprops}>{$elementname} - ".$this::emptyLink."</span>{$br}<br>\r\n";
					}
				}
				break;
			...
			default://текст
				...
				{
					if($element["itemText"]!=""){
						$rez.="<span title=\"{$elementname}\" class=\"linkicontxt hidedivlink linkicon {$adClass}\" id=\"{$idlink}\" data-id=\"{$element["id"]}\" data-iblock=\"67\"  data-section=\"{$defaultSection}\" data-xmlId=\"{$element["xmlId"]}\" {$sprops}>{$elementname}</span>\r\n";
						$rez.="<div style=\"{$adStyle} padding:1em;\" class=\"\" {$itemprop}>{$element["itemText"]}</div><br>\r\n";
					}else{
						$rez.="<span title=\"{$elementname}\" class=\"linkicontxt hidedivlink linkicon {$adClass}\" id=\"{$idlink}\" data-id=\"{$element["id"]}\" data-iblock=\"67\"  data-section=\"{$defaultSection}\" data-xmlId=\"{$element["xmlId"]}\" {$sprops}>{$elementname}</span>\r\n";
				...
				{
					if($element["itemText"]!=""){
						$rez.="<div {$itemprop} title=\"{$elementname}\" class=\"maindocselement\" id=\"{$idlink}\" data-id=\"{$element["id"]}\" data-iblock=\"67\"  data-section=\"{$defaultSection}\" data-xmlId=\"{$element["xmlId"]}\" {$sprops}>";
...
if( $obCache->InitCache($cacheLifetime, $cacheID, $cachePath) )
{
   $vars = $obCache->GetVars();// Извлечение переменных из кэша
	if(is_array($vars) && isset($vars["html"]))
   		$html=$vars["html"];
}
