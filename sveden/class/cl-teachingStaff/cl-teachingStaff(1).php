...
class teachingStaff extends  iasmuinfo{
	...
	function readUsersFromBD(){
		...
		$sql.="UF_ACADEMSTAT as academStat, UF_DEGREE as degree,UF_EMPLOYEEQUALIFICA as employeeQualification,";
	...
	public function generateItems(){
		...
		foreach($this->arUsers as $rsUser){
			...
			$this->listitems[$rsUser["ID"]]["employeeQualification"]=str_replace(array(";","\r\n"),"<br>",$rsUser["employeeQualification"]);
...
//echo"<pre>";	print_r($this->arUsers);	echo"</pre>";
	...
	private function prepareHtmlRucovodstvo(){
		$fieldsListHi=array(
			"num"=>array("№",2),
			"fio"=>array("Ф.И.О. препода&shy;вателя, реализу&shy;ющего прог&shy;рамму",6),
		...
		foreach($this->listitems as $userId=>$userRow){
			if($userRow["hipost"]){
				$hipost=true;
				$html.='<div class="maindocselementStaff" style="display:table-row;" id="rowHI'.$userId.'" data-iblock="7" data-id="'.$userId.'" title="'.$userRow["post"].'" itemprop="rucovodstvo">';
		...
		if(!$hipost){
				$html.='<div style="display:table-row;" class="maindocselementStaff" id="rowHI0" data-iblock="7" data-id="0" title="Руководитель организации" itemprop="rucovodstvo" >';
				foreach($fieldsListHi as $itemprop=>$c){
	...
	private function prepareHtmlRrucovodstvoZam(){
		...
		foreach($this->listitems as $userId=>$userRow){
			if($userRow["hizam"]){
				$hizam=true;
				$html.='<div style="display:table-row;" class="maindocselementStaff" id="rowHI'.$userId.'" data-iblock="7" data-id="'.$userId.'" title="'.$userRow["post"].'" itemprop="rucovodstvoZam" >';
				foreach($fieldsListHi as $itemprop=>$c){
					$sval=str_replace("&amp<br>amp","<br>",$userRow[$itemprop]);
					$html.='<div style="width:'.$c[1].'em; display:table-cell;"  class="prepodRowCell" itemprop="'.$itemprop.'">'.$sval.'</div>';
				}
		...
		if(!$hizam){
				$html.='<div style="display:table-row;" class="maindocselementStaff" id="rowHIZ0" data-iblock="7" data-id="0" title="Руководитель организации" itemprop="rucovodstvoZam" >';
				foreach($fieldsListHi as $itemprop=>$c){
					$html.='<div style="width:'.$c[1].'em; display:table-cell;" class="prepodRowCell" itemprop="'.$itemprop.'"  >'.$this::emptyCell.'</div>';
				}
				$html.='</div>';
	...
	private function prepareHtml(){
		...
		$fieldsList=array(
			...
			"teachingQual"=>array("Уровень обра&shy;зования",6),
			"teachingLevel"=>array("Квали&shy;фикация",6),
			"degree"=>array("Уч&#1104;ная степень педаго&shy;гического работника (при наличии)",6),
			"academStat"=>array("Уч&#1104;ное звание педаго&shy;гического работника (при наличии)",6),
			"employeeQualification"=>array("Наимено&shy;вание направ&shy;ления подготовки и (или) специаль&shy;ности педагоги&shy;ческого работника",8),
			"profDevelopment"=>array("Сведения о повышении квали&shy;фикации и (или) профес&shy;сиональной перепод&shy;готовке педагоги&shy;ческого работника (при наличии)",8),
			"genExperience"=>array("Общий стаж работы",5),
			"specExperience"=>array("Стаж работы по специальности",6),
		);
...
программы</h2>";
		...
		if(count($this->listitems)==0){
			$html.='<div  class="prepodrow maindocselementStaff" id="row0" data-id="0" data-iblock="7" title="'.$userRow["fio"].'" itemprop="teachingStaff">';
		...
		foreach($this->listitems as $userId=>$userRow){
			$html.='<div  itemprop="teachingStaff" class="prepodrow maindocselementStaff" id="row'.$userId.'" data-iblock="7" data-id="'.$userId.'" title="'.$userRow["fio"].'">';
				$rowNum++;
				foreach($fieldsList as $itemprop=>$RowCellData){
